...
class AgencyInfoExcelExporter {
  const AgencyInfoExcelExporter._();

  static Future<String> export(List<AgencyInfoEntity> items) async {
    Directory? directory;
    try {
    ...
    final timestamp = DateTime.now()
        ...
        .replaceAll(':', '-')
        .replaceAll('.', '-');
    final file = File('${directory.path}/agency-info-report-$timestamp.xls');
    await file.writeAsString(_buildWorkbook(items), flush: true);
    return file.path;
  }

  static String _buildWorkbook(List<AgencyInfoEntity> items) {
...
<head>
  ...
  <xml>
    <x:ExcelWorkbook>
      <x:ExcelWorksheets>
        <x:ExcelWorksheet>
          <x:Name>گزارش نمایندگی‌ها</x:Name>
          <x:WorksheetOptions>
