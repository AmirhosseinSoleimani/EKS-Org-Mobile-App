...
class VehicleInfoExcelReportFactory {
  ...
  static ExcelExportRequest create(List<VehicleInfoEntity> items) {
    return ExcelExportRequest.fromItems<VehicleInfoEntity>(
      ...
      items: items,
      columns: [
        ExcelExportColumn(title: 'نوع', valueBuilder: (item, _) => item.vehicleModelTitle, width: 22),
        ExcelExportColumn(title: 'انبارک', valueBuilder: (item, _) => item.isDepotEnabled == true ? 'فعال' : 'غیر فعال', width: 14),
        ExcelExportColumn(title: 'مدل تسهیم انبارک', valueBuilder: (item, _) => item.tashimType, width: 22),
        ...
        ExcelExportColumn(title: 'تاریخ نصب', valueBuilder: (item, _) => item.installTypeDateJalali ?? item.installTypeDate, width: 20),
        ExcelExportColumn(title: 'سریال IMEI', valueBuilder: (item, _) => item.imeiSerial, width: 22),
        ExcelExportColumn(title: 'دستگاه عیب یاب دارد', valueBuilder: (item, _) => item.isTroubleShooter == true ? 'دارد' : 'ندارد', width: 20),
