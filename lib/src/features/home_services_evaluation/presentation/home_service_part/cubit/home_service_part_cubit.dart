...
class HomeServicePartCubit extends Cubit<HomeServicePartState> {
  ...
  initState({required int laborIndex, required int serviceIndex}) async{
    ...
    if(activeServiceRequestResult != 'Success') {
      return emit(HomeServicePartState.error(dialogDataModel: DialogDataModel(title: '', description: activeServiceRequestResult)));
    }else if(lastEvaluationResult != 'Success') {
      return emit(HomeServicePartState.error(dialogDataModel: DialogDataModel(title: '', description: lastEvaluationResult)));
    } else {
  ...
  Future<void> selectPart({PartResponseEntity? partResponseEntity, EvaluationPartResponseEntity? evaluationPartEntity, bool? isEditablePart}) async{
    ...
    if(partMarkResult == 'Success') {
      ...
      return emit(HomeServicePartState.error(dialogDataModel: DialogDataModel(title: '', description: partMarkResult)));
    }
  }
    ...
    result.whenOrNull(
      success: (data, failures, resultCode) async {
        if (resultCode == 0) {
          ...
          emit(HomeServicePartState.error(dialogDataModel: DialogDataModel(title: 'خطا', description: failures?.listToString() ?? 'بروز خطا در سیستم')));
        }
      },
      failure: (NetworkExceptions error, msg) {
        emit(HomeServicePartState.error(
            dialogDataModel: DialogDataModel(title: 'خطا', description: msg ?? 'بروز خطا در سیستم')));
      },
    );
  }

  Future<void> getPart({required int laborIndex, required int serviceIndex})async{
    emit(const HomeServicePartState.loading());
  ...
  Future<void> submitCustomerPart({required int serviceIndex, required int laborIndex, required int partIndex, bool? isEditablePart}) async{
    ...
    if(selectPartResponseEntity?.mark == 'لطفا یک گزینه را انتخاب کنید') {
      emit(HomeServicePartState.notice(
  ...
  Future<void> submitPart({required int serviceIndex, required int laborIndex, required int partIndex, bool? isEditablePart}) async{
    ...
    HomeServiceEvaluationSecondStepCubit.customerServiceList?[serviceIndex].evaluationLabors?[laborIndex].parts?.add(
