...
class ServiceListCubit extends Cubit<ServiceListState> {
  ...
  ServiceListCubit(this._getServicesUseCase, this._getActiveServiceRequestUseCase, this._getLastEvaluationUseCase) : super(const ServiceListState.idle());
  ...
  final BehaviorSubject<List<EmdadServiceResultEntity>?> _serviceResponseSubject = BehaviorSubject<List<EmdadServiceResultEntity>?>();
  BehaviorSubject<List<EmdadServiceResultEntity>?> get serviceResponseSubject => _serviceResponseSubject;
  ...
  Future<bool> _getActiveServiceRequest() async {
    ...
    result.whenOrNull(
      ...
      failure: (NetworkExceptions error, msg) {
        emit(
          ServiceListState.error(dialogDataModel: DialogDataModel(title: '', description: msg ?? ''),),
        );
        resultValue = false;
      },
    );
  ...
  Future<void> getAllServices(String? value)async{
    ...
    final result = await _getServicesUseCase.call(ServiceRequestEntity(
    ...
    result.whenOrNull(
      success: (data, failures, resultCode) async {
        if (resultCode == 0) {
          _serviceResponseSubject.add(data?.emdadServices);
          emit(const ServiceListState.getServiceSuccess());
        } else {
          emit(ServiceListState.error(dialogDataModel: DialogDataModel(title: 'خطا', description: failures?.listToString() ?? 'بروز خطا در سیستم')));
        }
      },
      failure: (NetworkExceptions error, msg) {
        emit(ServiceListState.error(
  ...
  Future<void> selectService({required int index}) async{
    ...
    final evaluationEntity = EvaluationServiceEntity(
      ...
      serviceCategoryId: _serviceResponseSubject.valueOrNull?[index].serviceCategoryId,
      serviceCategoryCode: _serviceResponseSubject.valueOrNull?[index].serviceCategoryCode,
      serviceCategoryTitle: _serviceResponseSubject.valueOrNull?[index].serviceCategoryTitle,
      ...
      isSubscribedByNationalCode: _serviceResponseSubject.valueOrNull?[index].isSubscribedByNationalCode,
      serviceTypeTitle: _serviceResponseSubject.valueOrNull?[index].serviceTypeTitle,
      subscriptionId: _serviceResponseSubject.valueOrNull?[index].subscriptionId,
      limitationDescription: _serviceResponseSubject.valueOrNull?[index].limitedDescription,
      defectInfoProblemOrEzharCode: _serviceResponseSubject.valueOrNull?[index].workOrderCode,
      ...
      isImageMandatory: _serviceResponseSubject.valueOrNull?[index].imageMandatory,
    );
