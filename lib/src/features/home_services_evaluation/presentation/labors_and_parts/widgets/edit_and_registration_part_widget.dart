...
class EditAndRegistrationPartWidget extends BaseStatelessPage {
  const EditAndRegistrationPartWidget({super.key, this.partResponseEntity, this.isEditablePart, this.evaluationPartEntity, required this.serviceIndex, required this.laborIndex, required this.partIndex, this.changeCustomerPart, this.partCustomerIndex});
  ...
  Widget body(BuildContext context) {
    return BlocProvider(
      create: (_) {
        ...
        bloc.selectPart(partResponseEntity: partResponseEntity, evaluationPartEntity: evaluationPartEntity, isEditablePart: isEditablePart);
        return bloc;
      },
    ...
    return ListView(
      ...
      children: [
        ...
        EkButton(
          ...
          onTap: () {
            if(changeCustomerPart ?? false) {
              cubit.submitCustomerPart(serviceIndex: serviceIndex, laborIndex: laborIndex, partIndex: partCustomerIndex ?? 0,);
              return;
            }
            cubit.submitPart(laborIndex: laborIndex, serviceIndex: serviceIndex, partIndex: partIndex, isEditablePart: isEditablePart);
          },
        )
      ],
    );
  ...
  Widget _successPartPriceWidget({required BuildContext context}) {
    ...
    print(cubit.reusableSubject.value?.hasReusable);
    return Column(
      children: [
        EkTextField(
