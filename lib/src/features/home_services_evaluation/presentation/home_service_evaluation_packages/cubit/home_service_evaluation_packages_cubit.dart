...
import 'package:eks_sana_plus_org/src/features/home_services_evaluation/domain/usecase/fetch_insert_home_service_category_use_case.dart';
import 'package:eks_sana_plus_org/src/features/home_services_evaluation/domain/usecase/fetch_insert_home_service_package_use_case.dart';
import 'package:eks_sana_plus_org/src/features/home_services_evaluation/domain/usecase/fetch_insert_home_service_service_use_case.dart';
...
@injectable
class HomeServiceEvaluationPackagesCubit
    extends Cubit<HomeServiceEvaluationPackagesState> {
  final FetchInsertHomeServiceCategoryUseCase
  _fetchInsertHomeServiceCategoryUseCase;
  final FetchInsertHomeServicePackageUseCase
  _fetchInsertHomeServicePackageUseCase;
  final FetchInsertHomeServiceServiceUseCase
  _fetchInsertHomeServiceServiceUseCase;
  ...
  /// Behavior Subject
  final _homeServiceCategoryListSubject =
      BehaviorSubject<List<InsertHomeServiceCategoryResponseEntity?>>.seeded(
        [],
      );
  final _homeServicePackageListSubject =
      BehaviorSubject<List<InsertHomeServicePackageResponseEntity?>>.seeded([]);
  ...
  /// Stream
  Stream<List<InsertHomeServiceCategoryResponseEntity?>>
  get homeServiceCategoryListStream => _homeServiceCategoryListSubject.stream;

  Stream<List<InsertHomeServicePackageResponseEntity?>>
  get homeServicePackageListStream => _homeServicePackageListSubject.stream;

  Stream<ActiveServiceRequestResponseEntity?> get activeServiceRequestStream =>
      _activeServiceRequestSubject.stream;

  Stream<LastEvaluationEntity?> get lastEvaluationStream =>
      _lastEvaluationSubject.stream;
  ...
  Future<void> init() async {
    ...
    if (getLastEvaluationResult &&
        activeServiceRequestResult &&
        getHomeServiceCategoriesResult) {
        ...
        if (resultCode == 0) {
          ...
          emit(
            HomeServiceEvaluationPackagesState.error(
              message: failures?.listToString() ?? "خطای غیر منتظره",
            ),
          );
          resultValue = false;
        }
      ...
      success: (data, failures, resultCode) {
        if (resultCode == 0) {
          final updatedItem = oldItem.copyWith(
            homeServices: [...?oldItem.homeServices, ...?data?.services],
          );
          final updatedList =
              List<InsertHomeServiceCategoryResponseEntity?>.from(currentList);
  ...
  EvaluationServiceEntity mapToEvaluationService(
    InsertHomeServiceServiceItemResponseEntity service,
  ) {
  ...
  EvaluationLaborResponseEntity mapToEvaluationLabor(
    InsertHomeServiceLaborEntity labor,
  ) {
  ...
  EvaluationPartResponseEntity mapToEvaluationPart(
    InsertHomeServicePartEntity part,
  ) {
    return EvaluationPartResponseEntity(
      ...
      costCenterObject: part.partCostCenter != null
          ? CostCenterEntity(
  ...
  Future<void> validateServices() async {
    ...
    if (_homeServiceCategoryListSubject.valueOrNull?.isEmpty ?? true) {
      emit(
        HomeServiceEvaluationPackagesState.notice(
          message: 'لطفا یک سرویس را انتخاب نمائید',
        ),
      );
      return;
    }
    ...
    final validServices = rawList
        ...
        .where(
          (service) => service.labors != null && service.labors!.isNotEmpty,
        )
        .toList();
    List<EvaluationServiceEntity> mappedList = validServices
        .map(mapToEvaluationService)
        .toList();
    HomeServiceEvaluationSecondStepCubit.customerServiceList?.addAll(
      mappedList,
    );
    emit(const HomeServiceEvaluationPackagesState.validateServicesSuccess());
  }

  void toggleServiceCheckBox({
  ...
  void togglePartCheckBox({
    ...
    final updatedList = packageList.map((package) {
      ...
      final updatedGroups = package.partGroups?.map((group) {
        ...
        if (originalParts == null || partIndex >= originalParts.length) {
          return group;
        }
        ...
        final updatedParts = originalParts.asMap().entries.map((entry) {
          ...
          if (index == partIndex) {
            final shouldSelect = (package.isPartMandatory ?? false)
                ? true
      ...
      return package.copyWith(
        partGroups: updatedGroups
            ?.whereType<InsertHomeServicePartGroupEntity>()
            .toList(),
      );
    }).toList();
  ...
  void initializeMandatoryPartSelection() {
    ...
    final updatedList = currentList.map((pkg) {
      ...
      if (pkg.isPartMandatory == true &&
          (pkg.partGroups?.isNotEmpty ?? false)) {
        final firstPartGroup = pkg.partGroups?.first;
        final updatedParts = firstPartGroup?.parts?.asMap().entries.map((
          entry,
        ) {
          final index = entry.key;
          final part = entry.value;
  ...
  void addSelectedPackagesToHomeServices({
    required int categoryId,
    required int serviceId,
  }) {
    ...
    final updatedCategories = currentCategoryList.map((category) {
      ...
      final updatedServices = category?.homeServices?.map((service) {
        ...
        return service.copyWith(
          labors: [...filteredLabors, ...newLaborEntities],
        );
      }).toList();
      return category?.copyWith(
        homeServices: updatedServices
            ?.whereType<InsertHomeServiceServiceItemResponseEntity>()
            .toList(),
      );
    }).toList();
  ...
  void removedLaborFromService({
    ...
    final updatedList = currentList.map((category) {
      ...
      final updatedServices = category?.homeServices?.map((service) {
        if (service.id != serviceId) return service;
        final updatedLabors = (service.labors ?? [])
            .where((labor) => labor.laborId != laborId)
            .toList();
        final isSelected = updatedLabors.isNotEmpty
            ? service.isSelected
            : false;

        return service.copyWith(labors: updatedLabors, isSelected: isSelected);
      }).toList();
  ...
  InsertHomeServiceLaborEntity convertLaborToHomeServiceLabor(
    InsertHomeServicePackageResponseEntity? entity,
  ) {
  ...
  InsertHomeServiceLaborEntity convertLaborAndPartToHomeServiceLabor(
    InsertHomeServicePackageResponseEntity? entity,
  ) {
  ...
  Future<void> fetchHomeServicePackage({
    ...
    final result = await _fetchInsertHomeServicePackageUseCase.call(
      InsertHomeServicePackageRequestEntity(
        ...
        mobileNumber:
            _activeServiceRequestSubject.valueOrNull?.customerMobileNumber,
        serviceRequestId: _activeServiceRequestSubject.valueOrNull?.id,
      ),
    );
    result.whenOrNull(
      success: (data, failures, resultCode) async {
        if (resultCode == 0) {
          ...
          emit(
            const HomeServiceEvaluationPackagesState.servicePackageSuccess(),
          );
        } else {
          emit(
            HomeServiceEvaluationPackagesState.error(
