...
class CustomerInvoiceCubit extends Cubit<CustomerInvoiceState> {
  ...
  Future<void> fetchList({bool refresh = false}) async {
    ...
    emit(
      state.copyWith(
        status: initialLoad
            ? CustomerInvoiceViewStatus.initialLoading
            : CustomerInvoiceViewStatus.loadingMore,
    ...
    result.when(
      success: (page, failures, resultCode) {
        ...
        emit(
          state.copyWith(
            status: CustomerInvoiceViewStatus.loaded,
            items: records,
            totalCount: count,
  ...
  Future<void> loadPreview(InvoiceRecordEntity item) async {
    ...
    emit(
      state.copyWith(
        status: CustomerInvoiceViewStatus.previewLoading,
    ...
    result.when(
      success: (invoice, failures, resultCode) {
        emit(
          state.copyWith(
            status: CustomerInvoiceViewStatus.previewLoaded,
  ...
  Future<void> exportReport() async {
    ...
    final result = await _getCustomerInvoicesUseCase(
      _copyFilter(
        state.filter,
        pageSize: 0,
        skip: 0,
    ...
    await result.when<Future<void>>(
      success: (page, failures, resultCode) async {
        ...
        final exportResult = await _exportExcelUseCase(
          CustomerInvoiceExcelReportFactory.create(items),
        );
        ...
        exportResult.when(
          success: (data, failures, resultCode) {
            emit(
              state.copyWith(
                status: CustomerInvoiceViewStatus.reportSuccess,
                isReportLoading: false,
                successMessage: data.isBrowserDownload
                    ? 'دانلود گزارش فاکتورهای مشتری آغاز شد.'
                    : 'گزارش فاکتورهای مشتری ذخیره شد.',
                clearErrorMessage: true,
              ),
            );
          },
  ...
  void _emitError(
    ...
    emit(
      state.copyWith(
        status: CustomerInvoiceViewStatus.error,
        isInitialLoading: false,
        isPaginationLoading: false,
        isReportLoading:
